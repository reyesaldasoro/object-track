function [handles] = verifyThresholdObjects(handles,dataR,indexR,indexC,indexL,askUser)
%function [handles] = verifyThresholdObjects(handles,dataR,indexR,indexC,indexL,askUser)
%
%--------------------------------------------------------------------------
% verifyThresholdObjects  is used to display the datasets with the
...
maxIntensityProj                                        = max(dataR(:,:,indexLevels),[],3);
    ...
    % calculate the size of the Objects so far to fix min blob
...
if (askUser)
    ...
    if (strcmp(reply,'y'))||(strcmp(reply,'Y'))
        ...
        minBlob         = input (strcat('Minimum Size of Object acceptable = [',num2str(handles.minBlob),']; '));
        ...
        [handles] = verifyThresholdObjects(handles,dataIn,indexR,indexC,indexL);
        close all
    else
        close all
    end
