function [handles,numObjectsToSplit] = splitLargeObject(handles,dataRe,...
                                                  outlierThreshold)
%function [handles,numObjectsToSplit] = splitLargeObject(handles,dataRe,...
% outlierThreshold)
%
%--------------------------------------------------------------------------
% splitLargeObject  a routine to split all those cells that
...
if ~isfield(handles,'nodeNetwork')
    % positionObject calculates sphericity if third argument is
    % set to 1
    firstNetwork = positionObject(dataRe,[],0);
...
if (numFramesToProcess>1)
    disp('Split Large Objects and re-save in same folder  *_mat_La')
end
