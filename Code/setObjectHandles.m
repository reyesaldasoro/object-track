function handles = setObjectHandles(dataIn,handles,thresLevels,askUser)
%function handles = setObjectHandles(dataIn,handles)
%function handles = setObjectHandles(dataIn,handles,thresLevels)
%function handles = setObjectHandles(dataIn,handles,[],askUser)
%
%
%--------------------------------------------------------------------------
% setObjectHandles  determines some characteristics of the
...
if (exist('thresLevels','var'))
    ...
    %Modified to manage the variable askUser in verifyThresholdObjects
    if(~exist('askUser','var'))
        askUser = true; 
    end
    if (askUser)
        [handles] = verifyThresholdObjects(handles,dataIn); 
    end
