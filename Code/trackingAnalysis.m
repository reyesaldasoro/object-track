function [handles]= trackingAnalysis(dataInName,numLevelsToReduce,...
                                        dataStructure,thresLevels,minBlob)
%[handles]= trackingAnalysis()
%[handles]= trackingAnalysis(dataInName)
%[handles]= trackingAnalysis(dataInName,numLevelsToReduce)
%[handles]= trackingAnalysis(dataInName,numLevelsToReduce,...
%                               dataStructure)
%[handles]= trackingAnalysis(dataInName,numLevelsToReduce,...
...
%
%--------------------------------------------------------------------------
% trackingAnalysis is the main routine for the  object analysis
...
%                               in terms of the tracking of objects, and 
...
%     along with the objectTrack package.  If not, see 
%
...
%test the path to the objectTrack package it must be able to detect but the 
% m-files of objectTrack. 
% If they are not readable the path must be set first. 
testobjectTrack  = which('trackingAnalysis');

if (isempty(testobjectTrack))
    %the Path has not been set properly, display legend and exit.
    disp('')
    disp('---------------------------------------------------')
    disp('   objectTrack has not been properly configured')
    disp('---------------------------------------------------')
    disp('objectTrack requires a "Path" to all the relevant ')
    disp('files. To do this, go to: ')
    disp('    FILE > PATH > ADD FOLDER WITH SUBFOLDERS')
    disp('And then select the folder "objectTrack". ')
    disp('Matlab will add a number of folders to the list ')
    disp('in the window. Click SAVE and CLOSE.')
    disp('After this objectTrack is ready to be used.')
    ...
    clear testobjectTrack
end 

% Parsing of input arguments, if no data is received, call readObjects
% that will open a new window to locate the folder where the data is stored
if (nargin == 0)
    %No parameters, readObject shows a window when user can select
    %the data folder.
    [dataIn,handles] = readObjects();
    if isempty(handles)
            return;
    else
...
elseif (isempty(dataInName))
    disp('dataInName is empty, calling readObject to select data source...');
    [dataIn handles] = readObjects();
    if isemtpy(handles)
        return;
    else        
    ...
    if (strfind(dataInName,'_mat_'))
        ...
        %data volume. readObject allow to read those files and
...
%reduceObjects will read the folder and then iteratively reduce
...
if (isa(dataIn,'char'))
    ...
    if (processingLevel < 3)
        ...
        [dataR,handles] = reduceObject(dataIn,...
                                           numLevelsToReduce,...
                                           handles);
        %end
    end
    ...
    if (processingLevel < 5)
        ...
        if (~isfield(handles,'thresLevel'))
            % ---------- threshold and label the objects. ------------------
    ...
    if (processingLevel < 4)
        % fileName_mat_Re
        [dataL,handles] = thresObject(dataRe,handles);
        
        % -------- Split objects that are outliers by size  ----------------------
        [handles,numObjectsToSplit] = splitLargeObject(handles,dataLa,[]);
    end
    if (processingLevel < 5)
        
        if (~isfield(handles,'rows'))
            handles = setObjectHandles(dataIn,handles,[],false);                
        end
        ...
        if (~isfield(handles,'nodeNetwork') || size(handles.nodeNetwork,1) > 0)
            ...
            handles = trackObject(dataL,handles,0);
            
            %-- Find cases where a object disappears 1 frame, redraw and re-link --
            numObjectsChanged = linkLostObject(handles,dataRe);
            if numObjectsChanged>0
                handles2 = rmfield(handles,{'nodeNetwork','neighNetwork',...
                                    ...
                                    'reducedNetwork'});
                %-- with the new "lost" objects, track again -------------
                handles2 = trackObject(dataL,handles2,0);
            else
                handles2 = handles;
            end
            %--  Split objects that merge by collisions ----------------------
            numObjectsSplit = splitObjectCollision(handles2,dataRe);
            if numObjectsSplit>0
                handles3 = rmfield(handles2,{'nodeNetwork','neighNetwork',...
                                    'linkedNetwork','finalNetwork',...
                                    'distanceNetwork',...
                                    'finalLabel','reducedNetwork'});
                %-- with the new "lost" objects, track for a final time  ------
                disp('Final Tracking process');
                handles3 = trackObject(dataL,handles3,0);
            else
                handles3 = handles;
            end
