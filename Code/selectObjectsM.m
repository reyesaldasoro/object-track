function selectObjectsM(arg,handles,woundRegion)
%function selectObjectsM(arg,handles,woundRegion)
%--------------------------------------------------------------------------
% selectObjectsM  is a GUI based function that selects tracks and displays 
...
%--------------------------------------------------------------------------
    ...
    setappdata(fig,'selectObjectsM',state);
    setappdata(state.tool,'select3dhost',fig);

    set(fig,'windowbuttondownfcn','selectObjectsM(''click'')');
...
function off
    ...
    state = getappdata(gcbf,'selectObjectsM');
    ...
    if ~isempty(fig) & ishandle(fig)
        state = getappdata(fig,'selectObjectsM');     
...
function click
    ...
    setappdata(state.fig,'selectObjectsM',state);
...
function [fig, h] = createUI
    ...
    fig = figure('handlevisibility','off','menubar','none','resize','off',...
                 ...
                 pos,'deletefcn','selectObjectsM(''off'')');
